...
pub struct InitializeTweet<'info> {
    ...
    #[account(
        ...
        bump,
        constraint = topic.as_bytes().len() <= TOPIC_LENGTH @ TwitterError::TopicTooLong,
        constraint = tweet.content.as_bytes().len() <= CONTENT_LENGTH @ TwitterError::ContentTooLong
    )]
